...
const Login = ({ onLoginSuccess }) => {
    ...
    return setInterval(() => {
      ...
      lockedUsernames.forEach((username) => {
        if (lockedAccounts[username] > now) {
          ...
          updatedRemainingTimes[username] = Math.ceil((lockedAccounts[username] - now) / 1000);
          needsUpdate = true;
        } else {
          // Lock expired
      ...
      if (needsUpdate) {
        ...
        if (Object.keys(updatedLockedAccounts).length > 0) {
          localStorage.setItem("accountLocks", JSON.stringify(updatedLockedAccounts));
        } else {
    ...
    try {
      ...
      Object.keys(locks).forEach((username) => {
        ...
        if (lockTime > now) {
          ...
          if (username === formData.username) {
            setError(`Account "${username}" is locked. Try again in ${currentTimes[username]} seconds or use another account.`);
          }
        }
      });
  ...
  const lockAccount = (username, minutes) => {
    try {
      ...
      setError(`Too many failed attempts. Account "${username}" locked for ${minutes} minutes.`);
    } catch (e) {
  ...
  const handleChange = (e) => {
    ...
    if (name === "username" && value && lockedAccounts[value]) {
      setError(`Account "${value}" is locked. Try again in ${remainingTimes[value]} seconds or use another account.`);
  ...
  const handleSubmit = async (e) => {
    ...
    if (isCurrentUserLocked()) {
      setError(`Account "${formData.username}" is locked. Try again in ${getCurrentUserRemainingTime()} seconds or use another account.`);
      return;
    }
